...
<details>
  <summary>Правка от 19.05.2026</summary>
...
6. Мониторинг VPN-служб в Telegram-боте:
    * Добавлена обработка отсутствующих служб Антизапрета, чтобы временно удалённые службы во время обновления не считались «упавшими».
...
7. В скриптах `/scripts/` удалена жёсткая зависимость от `sudo`:
    * Обновлены скрипты `setup.sh`, `update.sh`, `uninstall.sh`, `change_passwd.sh`, `change_port.sh`.
    * Добавлена единая логика запуска привилегированных команд: если скрипт запущен от `root`, команды выполняются без `sudo`.
...
8. Исправлен расчет и отображение нагрузки ОЗУ на главной странице.
