...
<details>
  <summary>Правка от 03.09.2026</summary>

### StatusOpenVPN
1. Исправлена ошибка, из-за которой клиент мог подключиться с просроченным сертификатом. 
2. На страницах OpenVPN и WireGuard: 
    - контекстное меню заменено на отдельные иконки действий;
    - обновлены окна создания, продления и скачивания конфигураций.
4. Обновлена страница обновления (`settings/update`).
5. Добавлено определение внешнего IP через несколько источников .
6. В `setup.sh` / `ssl.sh`: неинтерактивный режим, сохранение HTTPS при действующем сертификате, учёт внешних nginx-конфигов, параметр `SERVER_URL`.
7. Обновлены описания параметров Antizapret (`/settings/install`).
