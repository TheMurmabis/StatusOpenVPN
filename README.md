...
<details>
  <summary>Правка от 17.05.2026</summary>

1. Короткие имена конфигов WireGuard/AmneziaWG:
    * Исправлено сохранение и применение опции «Короткие имена файлов» в веб-настройках.
    * На странице WireGuard скачивание `.conf` теперь учитывает эту опцию и выдаёт короткие имена файлов.
    * В Telegram-боте при включенной опции конфиг отправляется сразу с коротким именем, без дополнительного вопроса.
2. Гибкие ограничения протоколов для клиентов Telegram-бота:
    * Добавлены отдельные настройки доступа клиента к OpenVPN/WireGuard по интерфейсам `vpn` и `antizapret`.
    * Для совместимости сохранена поддержка старого формата настроек (`openvpn`/`wireguard`) с автоматическим преобразованием.
3. Управление доступами из меню админа в Telegram-боте:
    * В разделе привязок клиентов добавлено отдельное меню с переключателями доступных протоколов.
    * Кнопка удаления привязки вынесена в меню настройки конкретного клиента.
4. Ограничение выдачи конфигов по разрешённым протоколам:
    * В клиентском меню бота отображаются только доступные пользователю протоколы и интерфейсы.
    * При отсутствии доступных вариантов показывается отдельное уведомление.
    * Добавлены проверки доступа перед отправкой OpenVPN/WireGuard-конфигов с понятными сообщениями об отказе.
5. Улучшения отправки WireGuard-конфигов в Telegram:
    * Короткое имя файла теперь формируется в виде `<interface>-<client>-<type>.conf`.
    * Перед отправкой добавлены проверки существования, размера и непустого файла.
