...
1. Обновлён `ssl.sh`, улучшена работа с nginx и HTTPS. 
2. Доступ к приложению теперь осуществляется по адресу:  `https:<домен>/StatusOpenVPN/`.
...
### 3. Ручная настройка 
Если на одном домене размещено несколько веб-приложений, необходимо вручную добавить блок `location`
...
```
Блок `location /StatusOpenVPN/` должен быть размещён внутри `server {}` в текущей конфигурации nginx. 
...
После внесения изменений необходимо проверить конфигурацию и перезагрузить nginx:
