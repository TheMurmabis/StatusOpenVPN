...
<details>
  <summary>Правка от 26.04.2026</summary>

1. На главной странице:
    * в карточках OpenVPN и WireGuard добавлен счётчик заблокированных клиентов; если заблокированных нет, число и подпись не отображаются.
    * в карточке OpenVPN добавлен счётчик сертификатов, которые истекают менее чем через 30 дней; если таких сертификатов нет, счётчик не отображается.
2. На страницах со статистикой: 
    * Исправлено отображение графиков по часовому поясу клиента.
    * Для периодов «Неделя/Месяц/Год» переведён расчёт на календарные границы (с начала недели, месяца и года) в OpenVPN и WireGuard.
    * Обновлены подписи периода на графиках: для месяца показывается месяц и год, для года — только год.
    * Добавлены быстрые диапазоны «Последние 7/30/365 дней» в статистике OpenVPN и WireGuard.
3. На странице OpenVPN:
    * Добавлен фильтр по минимальному числу подключений клиента (с сохранением в браузере и кнопкой сброса).
    * На странице истории OpenVPN фильтрация переведена на серверный поиск `q` с сохранением запроса в пагинации.
    * Исправлено соответствие метрик «Получено/Передано» в графике и таблице OpenVPN-статистики (включая сортировку и итоги).
4. В настройках:
    * Добавлен лимит хранимых записей истории OpenVPN (`history_max_records`, 100–100000), и очистка БД логов теперь использует этот лимит.
    * Параметры сгруппированы по отдельным блокам `OpenVPN` и `WireGuard`.
    * Для OpenVPN и WireGuard добавлены чекбоксы управления отображением разделов в боковом и нижнем меню.
    * Для WireGuard добавлен чекбокс скрытия интерфейса `WARP`: интерфейс скрывается в списке клиентов и исключается из подсчёта клиентов WireGuard на главной странице.
...
6. Добавлен суточный диапазон статистики.
7. Реализована плавная отрисовка графиков.
8. Добавлен экспорт графиков (PNG/CSV).
9. Для недельного и месячного диапазонов добавлен выбор дня.
10. Добавлен выбор нескольких клиентов.
11. На странице "Клиенты OpenVPN" в итогах отображается суммарная скорость всех клиентов.
12. Удалена зависимость от CDN: все статические ресурсы (JS, CSS, шрифты) подключаются локально.
...
6. Исправлена логика запросов: если запросы у всех админов отключены, бот отвечает только добавленным клиентам и администраторам.
