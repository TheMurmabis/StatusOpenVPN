...
<details>
  <summary>Правка от 24.05.2026</summary>

### TelegramBot

1. В меню «Службы» (Меню сервера -> Службы) добавлены:
    * включение/отключение мониторинга;
    * отдельный выбор каждой службы для мониторинга.
2. В меню «Клиенты бота»:
    * добавлены отдельные ограничения по типам OpenVPN (auto/tcp/udp) и WireGuard (wg/am);
    * добавлено удобное отображение списка привязок;
    * исправлена обработка статусов клиентов и неактивных привязок.
3. В списках клиентов (OpenVPN/WireGuard -> Список клиентов):
    * добавлено отображение статуса клиента (онлайн/оффлайн/заблокирован);
    * при выборе клиента добавлены отображение краткой статистики, активности, а также функции блокировки/разблокировки.
