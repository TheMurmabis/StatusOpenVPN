...
<details>
  <summary>Версия от 15.03.2026</summary>

### StatusOpenVPN

1. Добавлена поддержка формата логов OpenVPN 2.7.0.
2. В `ssl.sh` добавлено создание самоподписанного сертификата для IP-адреса (создаётся, если домен не указан).
3. На странице **«Настройка»**:

   * Добавлена вкладка **«Общие»**.
   * Настройки Telegram-бота перенесены в новую вкладку.
   * Добавлена новая вкладка с журналом действий администраторов веб-интерфейса и бота.
4. На странице **«OpenVPN»**:

   * Галочка **«Показать IP»** перенесена в настройки в раздел **«Отображение IP-адресов»**.
   * На странице **«История»** добавлен постраничный просмотр (по 20 строк на страницу).
   * На странице **«Статистика»**:

     * Увеличен срок хранения данных до одного года (ранее — один месяц) [#35](https://github.com/TheMurmabis/StatusOpenVPN/issues/35).
     * Добавлен график и фильтры **неделя / месяц / год**.
5. На странице **«WireGuard»**:

   * Добавлена галочка временного отключения клиента.
   * Добавлена вкладка **«Статистика»** по аналогии со страницей **«OpenVPN»**.

### TelegramBot

1. Проведена общая оптимизация бота. Вся логика перенесена в отдельную директорию `src/tg_bot`.
2. Исправлен мониторинг нагрузки на сервер: уведомления теперь отправляются, если высокая нагрузка держится более 5 минут.
3. Добавлено автодобавление клиентов по кнопке [#36](https://github.com/TheMurmabis/StatusOpenVPN/issues/36): клиент отправляет запрос, администратор выполняет привязку.
4. Добавлено уведомление для клиентов бота о принятии или отклонении запроса.
5. В разделе **«Уведомления»** добавлена кнопка отключения запросов. Если она отключена у всех администраторов, новые запросы не принимаются.
