...
1. На страницах OpenVPN и WireGuard: 
    - контекстное меню заменено на отдельные иконки действий;
    - обновлены окна создания, продления и скачивания конфигураций.
2. Обновлена страница обновления (`settings/update`).
3. Добавлено определение внешнего IP через несколько источников .
4. В `setup.sh` / `ssl.sh`: неинтерактивный режим, сохранение HTTPS при действующем сертификате, учёт внешних nginx-конфигов.
5. Обновлены описания параметров Antizapret (`/settings/install`).
