...
<details>
  <summary>Версия от 14.03.2026</summary>

### StatusOpenVPN
1. Добавлена поддержка формата логов OpenVPN 2.7.0.
2. Добавлен самоподписной сертификат. 
...
1. Обновлен скрипт `ssl.sh` для корректного обновления конфигурации nginx. Доступ к приложению теперь осуществляется по адресу:  `https:<домен>/status/`.
2. Добавлена страница настроек с управлением Telegram-ботом и названием приложения.
3. Администраторы и привязки клиентов отображаются из `.env`.
4. Обновление Telegram-бота:
   - Клиентская сторона: клиенты могут получить свои конфиг файлы. Необходимо связать ID клиента телеграмм с именем клиента в формате: `client_id:имя_клиента` по команде `/client`.
   - Список админов, список онлайн клиентов.
   - Серверное меню со службами и онлайном.
   - Уведомления о нагрузке и настройка порогов CPU/RAM.
   - Кнопка перезагрузки сервера с подтверждением.
   - Новая команда `/id` - получение своего ID телеграмм.
